...
struct expression{
	...
	struct variable_function: public ::exprtk::ifunction<result_type> {
		std::string						variable_;
		Context*						context_;

		variable_function(const std::string& variable): ::exprtk::ifunction<result_type>(0), variable_(variable), context_(nullptr) { }

		variable_function(const variable_function& src) = default;
		variable_function& operator=(variable_function&& donor){
			variable_ = std::move(donor.variable_);
			return *this;
		}
		variable_function& operator=(const variable_function& donor){
			variable_ = donor.variable_;
			return *this;
		}
